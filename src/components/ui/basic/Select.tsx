	...
	({ ...props }) => {
		return (
			<HeadlessSelect.Trigger
				{...props}
				class={cn(
					"flex h-9 w-full items-center justify-between whitespace-nowrap rounded-md border border-input bg-transparent px-3 py-2 text-sm shadow-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-ring disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1 gap-2 cursor-pointer",
					props.class,
				)}
			>
				<Slot />
	...
	return (
		<HeadlessSelect.Item
			...
			class={cn(
				...
				"data-[highlighted]:border-base data-[highlighted]:bg-accent data-[highlighted]:cursor-pointer data-[highlighted]:text-accent-foreground",
				props.class,
			)}
		>
			<Slot />
